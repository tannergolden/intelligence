...
1. **Commit messages are Conventional Commits.** `type(scope): subject`, imperative mood, lower-case subject, no trailing period, with `!` before the colon for a breaking change. Types in use: `feat`, `fix`, `docs`, `style`, `refactor`, `perf`, `test`, `build`, `ci`, `chore`, `security`, `revert`.
