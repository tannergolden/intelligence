...
**The workflows join this gate on the next release, and not before.** GitHub Actions is a language CodeQL reads, so `.github/workflows/` and the `action.yml` under `actions/` can be analyzed as code rather than skipped as configuration. The upstream detection that emits that language landed after `v1.6.0`, which is where `@v1` currently points, so today this gate covers the Python and nothing else. The release that repoints the tag is what turns the rest on, with no edit here, which is the same mechanism every other shared gate on this page arrives through. Until then `actionlint` and `zizmor` in `lint-workflows.yml` remain the only readers of those files, and they check shape and known-bad patterns rather than following an expression to where it is expanded.
