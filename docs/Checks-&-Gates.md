...
**The workflows join this gate when the pin moves, and not before.** GitHub Actions is a language CodeQL reads, so `.github/workflows/` and the `action.yml` under `actions/` can be analyzed as code rather than skipped as configuration. The upstream detection that emits that language landed after `v1.6.0`, which is the release pinned in `codeql.yml`, so today this gate covers the Python and nothing else. Bumping the pin to a release carrying it is what turns the rest on, and that arrives as a pull request somebody reads rather than as a tag that moved underneath. Until then `actionlint` and `zizmor` in `lint-workflows.yml` remain the only readers of those files, and they check shape and known-bad patterns rather than following an expression to where it is expanded.
...
| [`lint-python.yml`](../.github/workflows/lint-python.yml) | ruff, over the two checkers and the packager          |
